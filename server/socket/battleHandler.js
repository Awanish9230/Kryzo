...
// In-memory state
const waitingQueue = []; // Array of { socketId, userId, rating, userName }
const activeRooms = new Map(); // roomId -> { p1, p2, question, startTime, scores }
const finishedRooms = new Map(); // roomId -> finishedRoomData (for rematches)
const rematchRequests = new Map(); // roomId -> Set(socketId)
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('update_progress', ({ roomId, progress }) => {
            ...
            if (progress === 100) {
                ...
                // Move to finishedRooms for rematches
                finishedRooms.set(roomId, { ...room, status: 'finished' });
                activeRooms.delete(roomId);

                // Auto-cleanup finished room after 5 minutes
                setTimeout(() => finishedRooms.delete(roomId), 5 * 60 * 1000);
            }
        });
        ...
        socket.on('leave_battle', ({ roomId }) => {
            ...
            io.to(roomId).emit('game_over', { winnerId, reason: 'opponent_left' });
        ...
        socket.on('join_battle_room', ({ roomId }) => {
            const room = activeRooms.get(roomId) || finishedRooms.get(roomId);
            if (!room) return;
        ...
        socket.on('rematch_request', async ({ roomId }) => {
            const room = finishedRooms.get(roomId);
            if (!room) return;
            ...
            if (requests.size >= 2) {
                ...
                if (questions.length > 0) {
                    ...
                    // Initialize new room with same players
                    const newRoomData = {
                        id: newRoomId,
                        p1: { ...room.p1, progress: 0 },
                        p2: { ...room.p2, progress: 0 },
                        question,
                        startTime: Date.now(),
                        status: 'active'
                    };
                    activeRooms.set(newRoomId, newRoomData);
        ...
        socket.on('disconnect', () => {
            ...
            for (const [roomId, room] of activeRooms.entries()) {
                if (room.p1.socketId === socket.id || room.p2.socketId === socket.id) {
                    ...
                    finishedRooms.set(roomId, { ...room, status: 'disconnected' });
                    activeRooms.delete(roomId);
                }
            }
        });
